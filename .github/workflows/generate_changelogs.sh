...
last_commit=$(curl -s https://api.github.com/repos/Owner/TG-FileStreamBot/tags | jq -r '.[0].commit.sha')

resp=$(curl -s https://api.github.com/repos/Owner/TG-FileStreamBot/commits?per_page=5)
data=$(echo "${resp}" | jq -c '.[]')
while read -r i; do
...
compare_url="https://github.com/Owner/TG-FileStreamBot/compare/$(echo $resp | jq -r '.[0].sha')...$last_commit"
